...
public class CartFragment extends BaseFragment {
	...
	TextView txtTruckNo;
	 @Override
	    public View onCreateView(LayoutInflater inflater, ViewGroup container,
	        ...
	        v.findViewById(R.id.btnChangeTruck).setOnClickListener(changeTruckListener);
	 ...
	 private void setList(){
		 CartItemsAdapter adapter = new  CartItemsAdapter(getActivity(), R.id.lstJobNos, Model.getInstance().getCart().items, this);
	...
	public boolean onOptionsItemSelected(android.view.MenuItem item) {
		if(item.getItemId() == R.id.action_add_items){
			...
			add(getActivity().getFragmentManager().beginTransaction(), new AddToOrderFragment());
			return false;
		}
		else if(item.getItemId() == R.id.action_submit){
			add(getActivity().getFragmentManager().beginTransaction(), new PrepCartFragment());
			return false;
		}
...
class CartItemsAdapter extends ArrayAdapter<CartItem>{
	...
	public CartItemsAdapter(Context context,
			int textViewResourceId, ArrayList<CartItem> objects, CartFragment fragment) {
	...
	OnItemClickListener listener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
			...
			cFragment.add(((Activity) context).getFragmentManager().beginTransaction(), fragment);
		}
	};
	@Override
