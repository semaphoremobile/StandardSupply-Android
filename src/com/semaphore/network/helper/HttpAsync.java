...
public class HttpAsync extends AsyncTask<SSOperation, Void, SSResponse>{
	 ...
	 * @param operation The operation whose request needs to be made. 
	 * Depending on the success or error the operations callback will be called 
	 * with 'callback(String result)' or 'error(String error)' methods
	 */
	...
	/**
	 * This would make an AsyncHttp call and fetch/post the data while the UIThread
	 * is not blocked
	 * 
	 * @param asyncHttpData
	 *            <p>
	...
	protected void onPostExecute(SSResponse result) {
		...
		try{
			Map<String, Object> map = gson.fromJson(result.result , new TypeToken<HashMap<String, Object>>() {}.getType());
			if(map.containsKey("auth_token") || result.operation.isAuthenticationRequired() == false){
				result.operation.getCallback().callback(result.result);
			}
			else if(map.containsKey("message")) {
				result.operation.getCallback().error(map.get("message").toString());
			}
			else{
	...
	private SSResponse doRequest(SSOperation operation) {
		...
		try {
			...
			if(jsonBody != null && baseRequest instanceof HttpEntityEnclosingRequestBase){
			...
			baseRequest.setHeader("Content-type", operation.getContentTypeHeader());
...
/**
 * Just needed to handle the result and status code and is passed on to the onPostExecute method.  
 */
class SSResponse{
