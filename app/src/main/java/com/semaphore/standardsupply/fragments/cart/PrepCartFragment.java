...
public class PrepCartFragment extends BaseFragment implements Observer {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View v = inflater
				.inflate(R.layout.fragment_prep_cart, container, false);
		...
		txtAddOrderNotes = (EditText) v.findViewById(R.id.txtOrderNotes);

		btnTapAddNotes = (Button) v.findViewById(R.id.btnAddOrderNotes);
		...
		btnTapAddNotes.setOnClickListener(addOrderNotesListener);
		return v;
	}
	...
	public boolean onOptionsItemSelected(android.view.MenuItem item) {
		if (item.getItemId() == R.id.action_next) {
			if (deliveryMethod == null) {
				...
				builder.setMessage("Please select a delivery method")
			...
			if (txtPO.getText().toString().equals("")) {
				...
				builder.setMessage("Please enter PO number").setNeutralButton(
						"OK", null);
			...
			if (!txtAddOrderNotes.getText().equals("")) {
				Model.getInstance().getCart().orderNotes = txtAddOrderNotes
						.getText().toString();
			}
			...
			if (deliveryMethod.equals(getString(R.string.will_call))) {
				add(getActivity().getFragmentManager().beginTransaction(),
						new SubmitCartFragment());
			} else {
				add(getActivity().getFragmentManager().beginTransaction(),
						new AddressesFragment(), "AddressesFragment");
			}
			return false;
		}
	...
	OnClickListener deliveryMethodListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			((TextView) v.findViewById(R.id.txtDelivery))
					.setTextColor(getResources().getColor(R.color.ss_green));

			// Set Will call cell
			btnWillCall.setBackgroundColor(getResources().getColor(
					R.color.light_gray));
			btnWillCall.findViewById(R.id.imgWillCall).setVisibility(
					View.INVISIBLE);
			((TextView) btnWillCall.findViewById(R.id.txtWillCall))
					.setTextColor(getResources().getColor(R.color.gray));

		}
	};

	OnClickListener addOrderNotesListener = new OnClickListener() {

		@Override
		public void onClick(View v) {

			txtAddOrderNotes.setVisibility(View.VISIBLE);
	...
	OnClickListener willCallMethodListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			((TextView) v.findViewById(R.id.txtWillCall))
					.setTextColor(getResources().getColor(R.color.ss_green));

			// Set Will call cell
			btnDelivery.setBackgroundColor(getResources().getColor(
					R.color.light_gray));
			btnDelivery.findViewById(R.id.imgDelivery).setVisibility(
					View.INVISIBLE);
			((TextView) btnDelivery.findViewById(R.id.txtDelivery))
					.setTextColor(getResources().getColor(R.color.gray));
	...
	public void update(Observable arg0, Object arg1) {
		...
		if (items == null || items.size() == 0) {
			// 1. Instantiate an AlertDialog.Builder with its constructor
			AlertDialog.Builder builder = new AlertDialog.Builder(
					PrepCartFragment.this.getActivity());
