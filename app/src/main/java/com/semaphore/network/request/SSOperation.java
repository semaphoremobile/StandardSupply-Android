...
/**
 * A base class for making requests to the servers. By default it return a http
 * method 'POST'. It can be changed by overriding the getHttpMethod() method.
 * Also by default it prefixes "http://162.242.148.122/api" to the action
 * required. In order to make a call to a different url override getActionUrl()
 * method. Similarly override getContentTypeHeader() to have a Content-Type
...
public class SSOperation {
    ...
    protected String getActionUrl() {
        /*String actionUrl = action == null ? ""
                : "http://206.72.198.49:3000/api" + action;*/
        String actionUrl = action == null ? "" :
                "http://stockmytruck.com/api" + action;
        Log.i("RequestResponse", "ActionUrl: " + actionUrl); // Staging:
        // 162.242.148.122
        return actionUrl;
    }
    ...
    public HttpRequestBase getRequestBase() {
        if (METHOD_GET.equals(getHttpMethod())) {
            ...
            try {
                Log.v("TAG", "GET***************TRY**************************");
                httpGet = new HttpGet(getActionUrl());
            } catch (Exception e) {
                Log.v("TAG", "GET*******************Exception**********************");
                Log.v("TAG", "Message ----------- " + e.toString());
                httpGet = null;
                //return null;
            }
            return httpGet;
    ...
    /**
     * If the server needs a json request, override this method to have a
     * HashMap of the params. The getRequestString() will take care of
    ...
    /**
     * Override it only if there is a need of non-json request to be made. If
     * the request is pure json use getRequest() method.
     *
     * @return Converts the Map<String,Object> returned by getRequest() into
     * json.
     */
