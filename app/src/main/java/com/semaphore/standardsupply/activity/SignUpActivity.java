...
public class SignUpActivity extends SSActivity implements Observer {
	...
	public void onSignUpClick(View v) {
		if (validate()) {
			...
			user.customer_id = Integer.parseInt(txtCustomerId.getText()
					.toString());
			user.default_branch_id = location.id;
			user.password = txtPassword.getText().toString();
			user.password_confirmation = txtPasswordConfirm.getText()
					.toString();
	...
	private boolean validate() {
		...
		String customer_id = txtCustomerId.getText().toString();
		if (!(isNumeric(customer_id) && (customer_id.length() == 6))) {
			validated = false;
			AlertDialog.Builder builder = new AlertDialog.Builder(
					SignUpActivity.this);

			builder.setMessage("Customer Id should be numerical 6 digits only");
			builder.setPositiveButton("OK",
					new DialogInterface.OnClickListener() {
						@SuppressLint("CommitTransaction")
						public void onClick(DialogInterface dialog, int id) {
