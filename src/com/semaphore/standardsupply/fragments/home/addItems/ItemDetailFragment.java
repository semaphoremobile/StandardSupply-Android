...
public class ItemDetailFragment extends BaseFragment implements Observer{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(getArguments() == null){
			...
			ScanHandler handler = new ScanHandler(ItemDetailFragment.this, barcode);
			handler.request();
		}
		else{
	...
	OnClickListener favoriteListener = new OnClickListener() {
		...
		public void onClick(View v) {
			FavoriteHandler handler = new FavoriteHandler(ItemDetailFragment.this, item);
			handler.request();
		}
	}; 
	...
	private void showChangeDialog(){
		...
		String locName =  location == null ? Settings.getLocation(getActivity()).name : location.name;
		// 2. Chain together various setter methods to set the dialog characteristics
		builder.setMessage("Your pickup location is: " + locName.toUpperCase(Locale.US) ).setTitle("");
		builder.setPositiveButton("click to continue", new DialogInterface.OnClickListener() {
		...
		builder.setNegativeButton("click to change", new DialogInterface.OnClickListener() {
			
			@Override
			public void onClick(DialogInterface arg0, int arg1) {
				if(Model.getInstance().getLocationsCache().items == null){
					//Load the locations cache
					Model.getInstance().getLocationsCache().addObserver(ItemDetailFragment.this);
	...
	private void showPODialog(){
		...
		// 2. Chain together various setter methods to set the dialog characteristics
		builder.setMessage("Please enter the po number:")
		.setTitle("");
		...
		builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int id) {
				...
				cart.location = location == null ? Settings.getLocation(getActivity()) : location;
				cart.truckId =  "" + Settings.getTruck(getActivity()).id;
				cart.PO = jNo;
				cart.items.add(cartItem);

				SubmitOrderHandler handler = new SubmitOrderHandler((SSActivity) getActivity());
				handler.cart = cart;
				handler.request();
			}
		});
	...
	OnClickListener addToCartListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if(Model.getInstance().defaultJobNo == null){
				...
				job.quantity = 1;
				CartItem cartItem = CartItem.cartItemForItem(ItemDetailFragment.this.item);
				cartItem.job_numbers.add(job);
	...
	public void receivedBarcode(String barcode) {
		...
		ArrayList<Item> itemms = Model.getInstance().getItemDetailCache(barcode).items;
		item = itemms.get(0);
		updateItem();
	}
	...
	public void onTabReselected() {
		...
		for (Iterator<BaseFragment> iterator = this.tabListener.getFragments().iterator(); iterator.hasNext();) {
